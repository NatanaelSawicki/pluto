package cryptohashing;

import enums.PlutoMessageDigestEnum;
import customexceptions.PlutoNoSuchAlgorithmException;
...
public class CryptoHandler {
    ...
    public static String applyMessageDigest(String in,
            PlutoMessageDigestEnum pEnum)
            throws PlutoNoSuchAlgorithmException {
        try {
            MessageDigest md = MessageDigest.getInstance(pEnum.getValue());
            md.update(in.getBytes());
            return StringHandler.stringHexa(md.digest());
        } catch (NoSuchAlgorithmException ex) {
            throw new PlutoNoSuchAlgorithmException(ExceptionHandler.getExceptionDetails(ex));
        }
    }
